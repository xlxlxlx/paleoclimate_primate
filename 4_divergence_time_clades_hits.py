...
protein_type_list = ["cgc"]
# add custom suffix to result filename
suffix = ''

df_clades_hits_fn = 'clades_gene_hits_'  
if high_thre_only:
    df_clades_hits_fn += '0.8' 
else :
...
for sequence_type in sequence_type_list:
    for protein_type in protein_type_list:
        ...
        fn = f"data/tblastn_summary_id{identity_threshold}_cover{coverage_threshold}_{sequence_type}_{protein_type}{suffix}.csv"
        ...
        for index, row in df_clades.iterrows():
            ...
            clade_dict[divergence_time] = [
                ...
                rank]
        for protein in df:
            species_hit = df[protein][df[protein] != ' '].index.tolist()
